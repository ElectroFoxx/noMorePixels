...
{
public:
	typedef const RenderEnvironmentPreset ConstRenderEnvironmentPreset;
...
typedef Ptr<const RenderEnvironmentPreset> ConstRenderEnvironmentPresetPtr;
typedef const RenderEnvironmentPreset ConstRenderEnvironmentPreset;
...
public:
	...
	struct StreamingResourcesStatistics
	{
		struct MemoryUsage
		{
			size_t usage{0};
	...
	{
		SystemInfo::MemoryStatistics cpu;
		SystemInfo::GPUMemoryStatistics gpu;

		StreamingResourcesStatistics textures;
		StreamingResourcesStatistics static_meshes;
		StreamingResourcesStatistics skinned_meshes;

		long long vram_small_pool_size{0};
		long long vram_small_pool_usage{0};

		int cpu_loading_resources{0};
		int gpu_loading_resources{0};

		long long ram_need{0};
		long long vram_need{0};
	};


	static const StreamingStatistics &getStreamingStatistics();
	...
	static Event<> &getEventBeginAuxiliarySurfaces();
	static Event<> &getEventEndAuxiliarySurfaces();
	static Event<> &getEventEndOpacityGBuffer();
	static Event<> &getEventBeginOpacityDecals();
	static Event<> &getEventEndOpacityDecals();
	static Event<> &getEventBeginAuxiliaryDecals();
	static Event<> &getEventEndAuxiliaryDecals();
	...
	static Event<> &getEventBeginWaterGBuffer();
	static Event<> &getEventEndWaterGBuffer();
	...
	static void setStreamingCommittedMemoryOvercommit(bool overcommit);
	static bool isStreamingCommittedMemoryOvercommit();
	...
	static void setStreamingCacheVRAM(int streamingcachevram);
	static int getStreamingCacheVRAM();
	static void setStreamingCacheRAM(int streamingcacheram);
	static int getStreamingCacheRAM();
	...
	static void setVREmulation(Render::RENDER_VR_EMULATION_MODE emulation);
	static Render::RENDER_VR_EMULATION_MODE getVREmulation();
	...
	static void setSharpenBlurSigma(float sigma);
	static float getSharpenBlurSigma();
	static void setSharpenBlurColorThreshold(float threshold);
	static float getSharpenBlurColorThreshold();
	static void setSharpenBlurDarkColorThresholdBias(float bias);
	static float getSharpenBlurDarkColorThresholdBias();
	static void setSharpenBlurRadius(int radius);
	static int getSharpenBlurRadius();
	static void setSharpenDiagonalSamples(bool samples);
	static bool isSharpenDiagonalSamples();
	static bool isUpscaleModeSupported(Render::RENDER_UPSCALE_MODE mode);
	...
	static void setReflectionDynamicAlphaFade(bool fade);
	static bool isReflectionDynamicAlphaFade();
	...
	static void setIndirectSpecularNormalization(bool normalization);
	static bool isIndirectSpecularNormalization();
...
public:
	...
	static void setBlendAlphaSrcFunc(int func);
	static int getBlendAlphaSrcFunc();
	...
	static int getBlendAlphaSrcFuncBuffer(int num);
	static void setBlendAlphaSrcFuncBuffer(int num, int src);
...
public:
	...
	static void setProjectionFullViewport(const Math::mat4 &viewport);
	static Math::mat4 getProjectionFullViewport();
