...
{
public:
	typedef const Viewport ConstViewport;
	...
	enum
	{
		...
		SKIP_STREAMING = 1 << 8,
		SKIP_AUTO_WHITE_BALANCE = 1 << 9,
		SKIP_AUTO_EXPOSURE = 1 << 10,
		SKIP_OCCLUSION_QUERY = 1 << 11,
	};
	...
	Event<> &getEventBeginAuxiliarySurfaces();
	Event<> &getEventEndAuxiliarySurfaces();
	Event<> &getEventEndOpacityGBuffer();
	Event<> &getEventBeginOpacityDecals();
	Event<> &getEventEndOpacityDecals();
	Event<> &getEventBeginAuxiliaryDecals();
	Event<> &getEventEndAuxiliaryDecals();
	...
	Event<> &getEventBeginWaterGBuffer();
	Event<> &getEventEndWaterGBuffer();
