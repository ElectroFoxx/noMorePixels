...
struct alignas(16) BoundSphere
{
	...
	UNIGINE_INLINE void set(const vec3 *points, int num_points, bool optimal)
	{
		...
		if (num_points > 2 && optimal)
		{
			...
			radius = (radius > 0.0f) ? sqrt(radius) : -1.0f;
		} else
		{
	...
	UNIGINE_INLINE bool compare(const BoundSphere &bs) const { return Math::compare(data, bs.data); }
	/// <summary>Compares the bounding sphere with the given one according to the degree of precision equal to 1.0e-6f.</summary>
	UNIGINE_INLINE bool operator==(const BoundSphere &bs) const { return compare(bs); }
	...
	{
		#ifdef USE_SSE
			return _mm_cvtss_f32(_mm_rcp_ss(_mm_rsqrt_ss(_mm_dp_ps(center.vec, center.vec, 0x7f)))) - radius;
		#else
	...
	UNIGINE_INLINE float distanceValid(const vec3 &point) const
	{
			...
			float distance = _mm_cvtss_f32(_mm_rcp_ss(_mm_rsqrt_ss(_mm_dp_ps(direction, direction, 0x7f))));
...
struct alignas(16) BoundBox
{
	...
	UNIGINE_INLINE void setTransform(const mat4 &transform)
	{
			...
			__m128 center = _mm_mul_ps(_mm_add_ps(minimum.vec, maximum.vec), _mm_half_ps);
	...
	UNIGINE_INLINE bool compare(const BoundBox &bb) const
	{
		return Math::compare(minimum, bb.minimum) && Math::compare(maximum, bb.maximum);
	}
	/// <summary>Compares the current bounding box with the given one.</summary>
	UNIGINE_INLINE bool operator==(const BoundBox &bb) const { return compare(bb); }
	...
	UNIGINE_INLINE bool insideValid(const BoundSphere &bs) const
	{
			...
			direction = _mm_sub_ps(_mm_mul_ss(radius, radius), _mm_dot3(direction, direction));
			return ((_mm_movemask_ps(direction) & 0x01) == 0);
		#else
			vec3 direction;
	...
	UNIGINE_INLINE float distanceValid() const
	{
			...
			return _mm_cvtss_f32(_mm_rcp_ss(_mm_rsqrt_ss(_mm_dot3(direction, direction))));
		#else
			vec3 direction;
			if (minimum.x > 0.0f)
...
struct alignas(16) BoundFrustum
{
	...
	UNIGINE_INLINE bool compare(const BoundFrustum &bf) const
	{
	...
	UNIGINE_INLINE bool operator==(const BoundFrustum &bf) const { return compare(bf); }
...
struct alignas(16) WorldBoundSphere
{
	...
	// equals
	/// <summary>Compares the bounding sphere with the given one. The degree of precision is equal to 1.0e-6f.</summary>
	UNIGINE_INLINE bool compare(const WorldBoundSphere &bs) const { return Math::compare(data, bs.data); }
	/// <summary>Compares the bounding sphere with the given one according to the degree of precision equal to 1.0e-6f.</summary>
	UNIGINE_INLINE bool operator==(const WorldBoundSphere &bs) const { return compare(bs); }
	/// <summary>Bounding spheres not equal comparison operator.</summary>
	UNIGINE_INLINE bool operator!=(const WorldBoundSphere &bs) const { return !compare(bs); }
...
struct alignas(16) WorldBoundBox
{
	...
	// compare
	/// <summary>Compares the current bounding box with the given one.</summary>
	UNIGINE_INLINE bool compare(const WorldBoundBox &bb) const
	{
		return Math::compare(minimum, bb.minimum) && Math::compare(maximum, bb.maximum);
	}
	/// <summary>Compares the current bounding box with the given one.</summary>
	UNIGINE_INLINE bool operator==(const WorldBoundBox &bb) const { return compare(bb); }
	/// <summary>Bounding boxes not equal comparison operator.</summary>
	UNIGINE_INLINE bool operator!=(const WorldBoundBox &bb) const { return !compare(bb); }
...
struct alignas(16) WorldBoundFrustum
{
	...
	// compare
	/// <summary>Compares the current bounding frustum with the given one.</summary>
	UNIGINE_INLINE bool compare(const WorldBoundFrustum &bf) const
	{
		if (camera != bf.camera) return false;
		if (bound_box != bf.bound_box) return false;
	...
	/// <summary>Bounding frustum equal comparison operator.</summary>
	UNIGINE_INLINE bool operator==(const WorldBoundFrustum &bf) const { return compare(bf); }
	/// <summary>Bounding frustum not equal comparison operator.</summary>
	UNIGINE_INLINE bool operator!=(const WorldBoundFrustum &bf) const { return !compare(bf); }
